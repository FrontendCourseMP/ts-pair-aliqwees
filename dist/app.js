...
function checkScobki(value) {
    ...
    if (!input.includes('(') && !input.includes(')') &&
        ...
        !input.includes('{') && !input.includes('}'))
        return 'false';
    const ok = cercle.open.length == cercle.close.length &&
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (e) => {
        e.preventDefault();
        const scheduledInput = document.getElementById('lab1_scheduledHour');
        const delayInput = document.getElementById('lab1_delayHours');
    ...
    form2.addEventListener('submit', (e) => {
        ...
        const lab2_value = document.getElementById('lab2_value');
        if (!form2 || !lab2_result)
            return;
